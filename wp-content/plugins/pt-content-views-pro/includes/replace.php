...
//			'is_posts_page',
	...
	function is_right_place( $query ) {
		...
		return !is_feed() && !is_admin() && $query->is_main_query() && $this->which_view && !$this->done && !defined( 'PT_CV_VIEW_OVERWRITE' );
	}
	...
	function modify_query( &$query, $view_settings ) {
		...
		CVP_Replace_Layout_Compatible::plugin_woocommerce_set_order( $query->query_vars );
	}
	...
	static function modify_query_params( $args ) {
		...
		return apply_filters( PT_CV_PREFIX_ . 'query_params_replacing', $args, $val );
	}
...
class CVP_Replace_Layout_Compatible {
	...
	static function init() {
		...
		add_filter( PT_CV_PREFIX_ . 'query_params', array( __CLASS__, 'blog_disable_sticky_posts' ) );
		add_filter( PT_CV_PREFIX_ . 'query_params_replacing', array( __CLASS__, 'plugin_goodlayers_not_suppress' ), 10, 2 );
	...
	static function blog_disable_sticky_posts( $args ) {
		if ( isset( $args[ 'cvp_replace_layout_page' ] ) && $args[ 'cvp_replace_layout_page' ] === 'home' && PT_CV_Functions::get_global_variable( 'current_page' ) > 1 ) {
	 ...
	 * @return array
	 */
	static function plugin_goodlayers_not_suppress( $args, $settings ) {
		$gdlr_active = defined( 'GDLR_CORE_LOCAL' ) || (function_exists( 'cv_is_active_plugin' ) && cv_is_active_plugin( 'goodlayers-core' ));
		if ( $gdlr_active && $settings === 'use_filter_settings' ) {
