	...
	class PT_CV_Hooks_Pro {
		...
		public static function filter_field_thumbnail_not_found( $args, $post, $dimensions, $gargs ) {
			...
			if ( self::_is_attachment( $post ) ) {
				$attachment = wp_get_attachment_image( $post->ID, $dimension_ready ? $dimensions : 'full', true, array( 'class' => $gargs[ 'class' ] ) );
				if ( $attachment ) {
		...
		public static function get_embed_video( $post, $dimensions, $content ) {
			if ( isset( $post->cvp_first_videoaudio ) ) {
				return $post->cvp_first_videoaudio;
			}

			// Get Media URL: Youtube, Vimeo, Dailymotion, Soundcloud
			$media_url = self::extract_video_url( $content, $post );

			return $post->cvp_first_videoaudio = self::embed_video( $media_url, $dimensions );
		}
		...
		public static function filter_field_href_attrs( $custom_attr, $open_in, $oargs = array() ) {
			...
			// Background image
			if ( PT_CV_Functions::get_global_variable( 'soft_resize' ) && !in_array( PT_CV_Functions::get_global_variable( 'view_type' ), array( 'scrollable', 'collapsible' ) ) ) {
		...
		public static function filter_field_href( $args, $post ) {
			...
			if ( isset( $other_settings[ 'open-in' ] ) ) {
				if ( $other_settings[ 'open-in' ] === PT_CV_PREFIX . 'none' ) {
					...
					if ( !empty( $post->cvp_first_videoaudio ) ) {
						return $post->cvp_first_videoaudio;
					}
					...
					if ( !self::_is_attachment( $post ) ) {
						if ( has_post_thumbnail( $post->ID ) ) {
							...
							if ( !$full_image ) {
								$full_image = self::get_embed_video( $post, array(), $content );
							}
						}
					} else {
		 ...
		 * @return array
		 */
		public static function multi_custom_field_sort( $args ) {
			...
			if ( $cvp_meta_query_order ) {
				if ( !isset( $args[ 'meta_query' ] ) ) {
					...
					$args[ 'meta_query' ] = array_merge( $args[ 'meta_query' ], $cvp_meta_query_order );
				}
			}
		...
		public static function filter_query_parameters( $args ) {
			...
			$args = CVP_Replace_Layout::modify_query_params( $args ); // call this before multi_custom_field_sort()

			PT_CV_Functions_Pro::filter_by_date( $args );
			$args	 = self::filter_by_custom_field( $args );
			$args	 = self::multi_custom_field_sort( $args );
						...
						foreach ( $dargs[ 'fields' ] as $field ) {
							...
							switch ( $field ) {
								case 'thumbnail':
									...
									break;
								case 'title':
									$field_html = PT_CV_Html_Pro::term_output_link( $term_link, esc_html( $term->name ), PT_CV_PREFIX . 'tao' );
				...
				if ( $sticky_option === 'prepend' || $sticky_option == 'prepend-all' ) {
					...
					if ( PT_CV_Functions::get_global_variable( 'current_page' ) === 1 ) {
						...
						if ( $content_items_more ) {
							$args = apply_filters( PT_CV_PREFIX_ . 'sticky_posts_handle', $content_items_more + $args, $content_items_more );
		...
		public static function filter_show_this_post( $args ) {
			...
			if ( $args && $membership_plugin && in_array( 'check_access_restriction', $advanced_settings ) ) {
				...
				if ( $membership_plugin === 'Ultimate Member' ) {
					if ( !PT_CV_UltimateMember::check_access() ) {
						$args = 0;
					}
