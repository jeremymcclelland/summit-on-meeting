...
class PT_Content_Views_Pro_Admin extends PT_Content_Views_Admin {
	...
	public function filter_orderby( $args ) {
		...
		$args[ 'common' ][ 'glossary-noorder' ] = array(
			'label'			 => array(
				'text' => '',
			),
			'extra_setting'	 => array(
			...
			'params'		 => array(
				array(
					'type'		 => 'html',
					'content'	 => sprintf( '<p class="cvp-notice">%s</p>', __( "Any sorts will NOT work with Glossary layout, because of Glossary items are always sorted and grouped by the first character of title.", 'content-views-pro' ) ),
				),
			),
			'dependence'	 => array( 'view-type', 'glossary' ),
		);
			...
			array(
				...
				'params'	 => array(
					array(
						...
						'std'	 => '',
						'desc'	 => sprintf( __( 'By default, whole page will be loaded (Header, Content, Footer). To load only Content, please enter selector to identify it %s', 'content-views-pro' ), '(for example: <code>#content</code> or <code>#main</code> or <code>.post</code> or <code>.hentry</code> or <code>article</code>)' ),
					),
				),
				'dependence' => array( $prefix . 'open-in', PT_CV_PREFIX . 'lightbox' ),
			),
		...
		$args[] = array(
			...
			'params' => array(
				array(
					...
					'options'	 => PT_CV_Values::yes_no( 'yes', __( 'Translate excerpt, do shortcodes and apply filters in excerpt', 'content-views-pro' ) ),
					'std'		 => '',
